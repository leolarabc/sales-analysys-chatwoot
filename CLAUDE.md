# FiestaHub Central de Inteligência — Dashboard 2.0

## Visão Geral

Dashboard multi-page HTML para a FiestaHub (empresa de festas e eventos em Belo Horizonte), com 5 abas: Comercial, Chatwoot Leads, ConversaScore, Meta Ads, Orgânico Social. Servido via nginx no Docker Swarm com Traefik como reverse proxy.
...
5. Integração com APIs: Chatwoot, FiestaHub, OpenAI Whisper, Claude API
...
- `FIESTAHUB_TOKEN` — Bearer token da API FiestaHub
...
# Deve retornar: {"status":"ok","service":"fiestahub-webhook"}
...
- Busca vendas e eventos da API FiestaHub
...
- Cruza com vendas FiestaHub para taxa de conversão
...
- **Assunto**: `[FiestaHub ERRO] nome_do_script — DD/MM/YYYY HH:MM`
...
```
0 8,12,17 * * * cd /home/your-project && python3 collectors/comercial.py >> /var/log/fiestahub-comercial.log 2>&1
5 8,12,17 * * * cd /home/your-project && python3 collectors/chatwoot_leads.py >> /var/log/fiestahub-chatwoot.log 2>&1
0 6 * * * cd /home/your-project && python3 collectors/scoring.py >> /var/log/fiestahub-scoring.log 2>&1
```

Verificar logs:
```bash
tail -f /var/log/fiestahub-comercial.log
tail -f /var/log/fiestahub-chatwoot.log
tail -f /var/log/fiestahub-scoring.log
```
...
```bash
tail -20 /var/log/fiestahub-comercial.log
python3 /home/your-project/collectors/comercial.py
```

### Score não gera
```bash
tail -20 /var/log/fiestahub-scoring.log
